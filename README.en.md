...
Compatibility runs select only automatic exact sources whose current commit passed static checks. The install phase uses the official DSH CLI with `--ignore-scripts` and mounts only a disposable directory. The runtime phase has no network, tokens, extra capabilities, Docker socket, or maintainer workspace. It starts a clean DSH baseline before the plugin Profile and verifies bounded startup and SIGTERM disposal. Harness v1 does not yet execute the Mock Agent Loop, browser React mount, or old-version update rollback, so those checks remain explicitly `unsupported` or `inconclusive`; the overall result is `partial`, never a fabricated full pass.
