...
var JobTable = class {
  ...
  seq = 0;
  create(kind, packageName, allowQueued = false) {
    ...
    while (this.jobs.size > MAX_JOBS) {
      ...
      if (oldestFinished === void 0) break;
...
var ProfileMutationQueue = class {
...
var _restart_dec, _diagnoseConflicts_dec, _setAgentWorkspaceDir_dec, _agentWorkspace_dec, _setInstallDir_dec, _installLocation_dec, _installed_dec, _jobStatus_dec, _setEnabled_dec, _uninstall_dec, _updateBatch_dec, _update_dec, _manualInstall_dec, _installPlugin_dec, _guidedTask_dec, _details_dec, _search_dec, _a, _init;
var MarketplaceService = class extends (_a = TypertRemoteService, _search_dec = [Remote("search")], _details_dec = [Remote("details")], _guidedTask_dec = [Remote("guidedTask")], _installPlugin_dec = [Remote("installPlugin")], _manualInstall_dec = [Remote("manualInstall")], _update_dec = [Remote("update")], _updateBatch_dec = [Remote("updateBatch")], _uninstall_dec = [Remote("uninstall")], _setEnabled_dec = [Remote("setEnabled")], _jobStatus_dec = [Remote("jobStatus")], _installed_dec = [Remote("installed")], _installLocation_dec = [Remote("installLocation")], _setInstallDir_dec = [Remote("setInstallDir")], _agentWorkspace_dec = [Remote("agentWorkspace")], _setAgentWorkspaceDir_dec = [Remote("setAgentWorkspaceDir")], _diagnoseConflicts_dec = [Remote("diagnoseConflicts")], _restart_dec = [Remote("restart")], _a) {
  constructor(ctx, config) {
    ...
    __publicField(this, "mutationQueue", new ProfileMutationQueue());
  ...
  async installed() {
    try {
      ...
      await Promise.all(entries.map(async (entry) => {
        ...
        entry.updateAvailable = versionOrder > 0 || versionOrder === 0 && registered.install.source === "github" && isGitHubSpec2(entry.currentSpec) && !entry.currentSpec.toLocaleLowerCase().includes(registered.verifiedCommit.toLocaleLowerCase());
  ...
  async startJob(kind, repo, ref, allowQueuedMutation = false) {
    if (this.restartPending) {
      return fail("restart-pending", "DSH is already preparing to restart.");
    }
