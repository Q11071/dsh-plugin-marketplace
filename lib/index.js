...
import { z } from "zod";
var PAGE_SIZE = 30;
var registryPluginSchema = z.object({
...
var _installed_dec, _jobStatus_dec, _uninstall_dec, _update_dec, _installPlugin_dec, _details_dec, _search_dec, _a, _init;
var MarketplaceService = class extends (_a = TypertRemoteService, _search_dec = [Remote("search")], _details_dec = [Remote("details")], _installPlugin_dec = [Remote("installPlugin")], _update_dec = [Remote("update")], _uninstall_dec = [Remote("uninstall")], _jobStatus_dec = [Remote("jobStatus")], _installed_dec = [Remote("installed")], _a) {
  ...
  async update(request) {
    return this.startJob("update", request.repo, request.ref ?? "", () => void 0);
  }
  ...
  async installed() {
    try {
      const profile = profileLocation(this.ctx);
      return ok({ entries: installedEntries(readProfileManifest2(NAME2, profile.dir), profile.dir) });
    } catch (error) {
      return toFailure(error);
    }
  }
  ...
  async drive(job, profile, args, before) {
    try {
