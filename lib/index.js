...
function parsePackagePath(value) {
  ...
  if (normalized.length > 512 || normalized.startsWith("/") || normalized.includes("\\") || normalized.includes("\0") || normalized.split("/").some((segment) => segment === "" || segment === "." || segment === "..")) {
...
var GitHubClient = class {
  ...
  async details(repoSpec, ref, requestedPackagePath = "") {
    ...
    return {
      ...
      manifest,
      patch,
      readmeUrl: packagePath === "" ? "https://github.com/" + owner + "/" + repo + "#readme" : "https://github.com/" + owner + "/" + repo + "/tree/" + resolved.ref + "/" + encodeRepoPath(packagePath) + "#readme",
      rate: resolved.rate
    };
  }
};
...
function safePackagePath(value) {
  return value === "" || value.length <= 512 && !value.startsWith("/") && !value.includes("\\") && !value.includes("\0") && value.split("/").every((segment) => segment !== "" && segment !== "." && segment !== "..");
}
...
var RegistryClient = class {
  ...
  async search(query, page, sort) {
    ...
    filtered.sort((left, right) => {
      const primary = sort === "updated" ? Date.parse(right.updatedAt) - Date.parse(left.updatedAt) : right.stars - left.stars;
  ...
  async findByPackage(packageName, currentSpec = "") {
    ...
    if (source !== null) {
      return matches.find((plugin) => plugin.fullName.toLocaleLowerCase() === source.repo && plugin.packagePath.toLocaleLowerCase() === source.packagePath) ?? matches[0];
    }
    return matches[0];
  }
  async load() {
...
function normalizeRegistry(raw) {
  ...
  if (version === 3) return registryV3Schema.parse(raw);
  if (version === 2) {
    const legacy2 = registryV2Schema.parse(raw);
    return {
      schemaVersion: 3,
      generatedAt: legacy2.generatedAt,
      plugins: legacy2.plugins.map((plugin) => withRootIdentity(plugin))
    };
  }
...
function withRootIdentity(plugin) {
  return { ...plugin, id: plugin.fullName, packagePath: "" };
}
function registryId(repo, packagePath) {
  return packagePath === "" ? repo : repo + "&path:/" + packagePath;
}
function githubSpec(repo, commit, packagePath) {
...
var MarketplaceService = class extends (_a = TypertRemoteService, _search_dec = [Remote("search")], _details_dec = [Remote("details")], _installPlugin_dec = [Remote("installPlugin")], _update_dec = [Remote("update")], _uninstall_dec = [Remote("uninstall")], _setEnabled_dec = [Remote("setEnabled")], _jobStatus_dec = [Remote("jobStatus")], _installed_dec = [Remote("installed")], _a) {
  ...
  async installed() {
    try {
      ...
      await Promise.all(entries.map(async (entry) => {
        ...
        entry.canUpdate = registered.install.mode === "automatic" && registered.install.source === "github" && registered.install.profiles.includes(profile.name) && registered.install.spec !== "";
      }));
  ...
  async startJob(kind, repo, packagePath, ref, gate) {
    try {
...
function executableSpec(plugin) {
  if (plugin.install.source === "github") {
    const expected = "github:" + plugin.fullName + "#" + plugin.verifiedCommit + (plugin.packagePath === "" ? "" : "&path:/" + plugin.packagePath);
