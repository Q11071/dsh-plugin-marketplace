...
var securityReportSchema = z.object({
  schemaVersion: z.literal(1),
...
var compatibilityReportSchema = z.object({
  schemaVersion: z.literal(1),
  results: z.array(z.object({
    repository: z.string().regex(/^[\w.-]+\/[\w.-]+$/),
    verifiedCommit: z.string().regex(/^[0-9a-f]{40}$/i),
...
var RegistryClient = class {
  ...
  async loadSource(source) {
    ...
    if (url.protocol === "file:") {
      ...
      if (response.status === 304 && this.cache?.source === source) {
        const registry2 = await this.applyVerificationSidecars(this.cache.registry, source);
        this.cache = { ...this.cache, registry: registry2, expiresAt: Date.now() + this.cacheMs };
        return registry2;
      }
  ...
  async applyVerificationSidecars(registry, source) {
  ...
  async loadOptionalCompanion(source, filename, schema) {
    try {
...
function applyVerification(registry, security, compatibility) {
  ...
  return {
    ...
    plugins: registry.plugins.map((plugin) => {
      ...
      return {
        ...plugin,
        verification: {
          security: securityRow?.verifiedCommit === plugin.verifiedCommit ? { status: securityRow.status, checkedAt: securityRow.scannedAt } : { status: "pending", checkedAt: null },
          compatibility: compatibilityRow?.verifiedCommit === plugin.verifiedCommit ? { status: compatibilityRow.result, checkedAt: compatibilityRow.checkedAt } : { status: "pending", checkedAt: null }
        }
      };
    })
  };
...
function profileLocation(ctx) {
  ...
  if (baseUrl !== void 0) {
    ...
    const dir = /\.(yml|yaml|json)$/.test(basename(raw)) ? dirname2(raw) : raw;
...
function installedEntries(manifest, dir, pluginDir) {
  ...
  for (const packageName of Object.keys(manifest.dependencies ?? {})) {
    ...
    entries.push({
      ...
      isBundle,
      linked: true,
      location: manifestPath === null ? "" : dirname2(manifestPath),
...
function defaultAgentWorkspaceDir() {
  return join3(dirname3(marketplaceSettingsPath()), "agent-workspace");
}
...
function linkProfilePeerDependencies(target, profileDir) {
  ...
  for (const packageName of Object.keys(peers)) {
    ...
    const candidates = providedNames.flatMap((providedName) => [
      ...
      dependencyPath(join3(dirname3(profileDir), "node_modules"), providedName)
    ]);
...
function createProfilePackageLink(profileDir, packageName, target, jobId) {
  ...
  mkdirSync(dirname3(linkPath), { recursive: true });
...
function packagePatchPath(packageName, dir) {
  ...
  try {
    ...
    return join4(dirname4(manifestPath), patch);
  } catch {
    return null;
  }
}
function packageEntryPath(packageName, dir) {
  ...
  try {
    ...
    return join4(dirname4(manifestPath), entry);
  } catch {
    return null;
  }
}
...
var _restart_dec, _diagnoseConflicts_dec, _setAgentWorkspaceDir_dec, _agentWorkspace_dec, _setInstallDir_dec, _installLocation_dec, _installed_dec, _jobStatus_dec, _setEnabled_dec, _uninstall_dec, _update_dec, _manualInstall_dec, _installPlugin_dec, _guidedTask_dec, _details_dec, _search_dec, _a, _init;
