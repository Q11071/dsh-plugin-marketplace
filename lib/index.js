...
var JobTable = class {
  ...
  list() {
    return [...this.jobs.values()].sort((left, right) => left.startedAt - right.startedAt).map((job) => this.snapshot(job));
  }
  hasActive() {
  ...
  async installed() {
    try {
      ...
      await Promise.all(entries.map(async (entry) => {
        ...
        entry.updateAvailable = versionOrder > 0 || versionOrder === 0 && registered.install.source === "github" && /^(?:github:|git\+https:\/\/github\.com\/|https:\/\/github\.com\/)/i.test(entry.currentSpec) && !entry.currentSpec.toLocaleLowerCase().includes(registered.verifiedCommit.toLocaleLowerCase());
