...
function buildGuidedAgentTask(plugin, profile, operation, evidence) {
  ...
  const prompt = [
    ...
    "6. \u5982\u679C\u65E0\u6CD5\u8BC1\u660E\u5B89\u88C5\u6E90\u3001\u5305\u8EAB\u4EFD\u3001Profile \u517C\u5BB9\u6027\u6216\u8FD0\u884C\u4EA7\u7269\u5B89\u5168\uFF0C\u5C31\u505C\u6B62\u5E76\u89E3\u91CA\u7F3A\u5931\u8BC1\u636E\uFF0C\u4E0D\u8981\u731C\u6D4B\u6216\u7ED5\u8FC7 Registry\u3002",
    "",
    `${verb}\u4E0E\u9A8C\u6536\u8981\u6C42\uFF1A`,
...
import { basename as basename2, dirname as dirname2, join as join3, relative, resolve } from "node:path";
...
function readMarketplaceSettings() {
  try {
    ...
    const pluginRoots = Array.isArray(parsed.pluginRoots) ? parsed.pluginRoots.filter((value) => typeof value === "string" && value.trim() !== "").map((value) => resolve(value)) : [];
...
function defaultPluginRoot(profileDir) {
  return join3(resolve(profileDir), "node_modules");
}
...
function pluginFolderName(packageName) {
...
function pluginTarget(profile, packageName) {
  return join3(profile.pluginDir, pluginFolderName(packageName));
}
...
function isManagedPluginTarget(profile, packageName, target) {
  ...
  if (basename2(resolvedTarget).toLocaleLowerCase() !== expectedFolder) return false;
  if (!knownPluginRoots(profile).some((root) => dirname2(resolvedTarget).toLocaleLowerCase() === root)) return false;
  try {
    const manifest = JSON.parse(readFileSync3(join3(resolvedTarget, "package.json"), "utf8"));
...
function linkProfilePeerDependencies(target, profileDir) {
  ...
  for (const packageName of Object.keys(peers)) {
    ...
    if (existsSync2(destination)) continue;
...
var _restart_dec, _diagnoseConflicts_dec, _setInstallDir_dec, _installLocationRemote_dec, _installed_dec, _jobStatus_dec, _setEnabled_dec, _uninstall_dec, _update_dec, _installPlugin_dec, _guidedTask_dec, _details_dec, _search_dec, _a, _init;
var MarketplaceService = class extends (_a = TypertRemoteService, _search_dec = [Remote("search")], _details_dec = [Remote("details")], _guidedTask_dec = [Remote("guidedTask")], _installPlugin_dec = [Remote("installPlugin")], _update_dec = [Remote("update")], _uninstall_dec = [Remote("uninstall")], _setEnabled_dec = [Remote("setEnabled")], _jobStatus_dec = [Remote("jobStatus")], _installed_dec = [Remote("installed")], _installLocationRemote_dec = [Remote("installLocation")], _setInstallDir_dec = [Remote("setInstallDir")], _diagnoseConflicts_dec = [Remote("diagnoseConflicts")], _restart_dec = [Remote("restart")], _a) {
  ...
  async startJob(kind, repo, ref, gate) {
    ...
    try {
      ...
      const target = profile.custom ? kind === "update" ? installedPluginTarget(profile, packageName, before) : pluginTarget(profile, packageName) : profilePackagePath(profile.dir, packageName);
  ...
  async driveInstall(job, profile, spec, before, requiresRestart = true) {
    if (!profile.custom) {
    ...
    const manifestPath = join5(profile.dir, "package.json");
    const originalManifest = readFileSync5(manifestPath, "utf8");
    ...
    try {
      ...
      mkdirSync2(profile.pluginDir, { recursive: true });
      ...
      if (manifestWritten) writeFileSync2(manifestPath, originalManifest, "utf8");
  ...
  async driveProfileInstall(job, profile, spec, before, requiresRestart = true) {
    const stageDir = join5(dirname4(marketplaceSettingsPath()), "staging", job.jobId);
    const manifestPath = join5(profile.dir, "package.json");
    const lockfilePath = join5(profile.dir, "pnpm-lock.yaml");
    const originalManifest = readFileSync5(manifestPath, "utf8");
    const originalLockfile = existsSync3(lockfilePath) ? readFileSync5(lockfilePath, "utf8") : null;
    ...
    try {
      ...
      if (profileAttempted) {
        writeFileSync2(manifestPath, originalManifest, "utf8");
        if (originalLockfile === null) rmSync2(lockfilePath, { force: true });
        else writeFileSync2(lockfilePath, originalLockfile, "utf8");
  ...
  async driveUninstall(job, profile, before, requiresRestart = true) {
    if (!profile.custom) {
    ...
    try {
      ...
      const next = JSON.parse(originalManifest);
      if (next.dependencies !== void 0) delete next.dependencies[job.packageName];
      ...
      if (managedTarget && existsSync3(target)) rmSync2(target, { recursive: true, force: true });
  ...
  async driveProfileUninstall(job, profile, before, requiresRestart = true) {
    const manifestPath = join5(profile.dir, "package.json");
    const lockfilePath = join5(profile.dir, "pnpm-lock.yaml");
    const originalManifest = readFileSync5(manifestPath, "utf8");
    const originalLockfile = existsSync3(lockfilePath) ? readFileSync5(lockfilePath, "utf8") : null;
...
__decorateElement(_init, 1, "installLocationRemote", _installLocationRemote_dec, MarketplaceService);
