...
- 新增 Actions 一键全量扫描模式：一次手动触发后按每轮 200 个目标自动接力，报告作为断点，后续轮次跳过重复发现和已经验证的精确 commit；支持 API 额度、最大轮次及连续无进度保护性暂停。
- 新增 `registry/full-scan-state.json` 全量扫描状态，记录会话、轮次、安全/兼容性剩余工作、API 余额和暂停原因；中断后可从已提交报告继续。
- 将仓库发现、安全扫描和运行时兼容性验证聚合为同一条两小时工作流；本轮安全通过的精确 commit 会立即进入本轮兼容性验证，不再跨工作流等待。
- 安全门现在保存分类器原始安装决策，待结果通过后原样恢复；存量回填优先处理可自动安装候选。
- 新增运行时兼容性阶段与 `registry/compatibility-report.json`：只对静态检查通过的精确自动安装来源，使用官方 DSH CLI 禁用生命周期脚本安装，并在断网临时 Profile 中检查 Host 启动和释放。
- Agent 类插件现在会在断网 Profile 中运行市场自有的确定性 Mock Agent 回路，验证请求、工具调用、`tool/result`、调用 ID、最终消息与释放；不会执行仓库提供的测试脚本。
- Client Bundle 只允许解析 DSH 官方平台模块并验证 ModuleLoader factory 与 Cordis 插件导出；尚未执行的浏览器 React 挂载和无法适用的更新回滚仍明确显示为部分验证。
- 插件市场卡片分别显示静态检查与运行时兼容性状态，不再共用模糊的“已验证”标签。
- 新增按精确 commit 增量运行的插件安全工作流：静态检测反向 Shell、破坏性命令、持久化、
  矿工、编码载荷、凭据外传组合、下载执行组合和原生可执行文件。
- 静态检查通过后，在无网络、只读、无额外 capability 且限制 CPU、内存和 PID 的临时
  Docker 容器中探测 Host 入口；第三方代码任务不接收 PAT 或仓库写凭据。
- 新增 `registry/security-report.json`，新收录及 commit 变化的插件在安全结果产生前保持
  引导安装，存量插件通过有界队列逐批回填。

### 修复

- 修复 Registry 扫描与引导审计在同一轮中对刚发布 npm 版本得到不同结果时，校验直接失败
  的问题；第二轮精确 tarball 证据现在会安全回写并将条目晋升为自动安装。
- 引导审计现在尊重作者人工步骤、冲突证据和安全复核原因，不再把这些条目误报为可自动安装。
- GitHub API 配额在候选验证中途耗尽时，本轮已完成结果会正常写回，其余候选留待下轮重试，
  不再因为等待时间超过 90 秒而丢弃整轮进度。
- 引导审计改为从精确 commit 的 Raw URL 读取 README，不再与候选发现共享 PAT 的 API 配额。
- 单仓库 README 超出大小上限或发生临时网络错误时，该条目保持引导并记录审计不完整，
  不再导致整个 Registry 工作流失败。
- 安全沙箱现在会将“精确源码没有提交运行入口”和“缺少运行依赖”标为无法判定，
  不再误报成运行失败；扫描器版本升级后会优先复验旧结果。
- 持久化检测要求同时存在计划任务/自启动信号与进程执行能力，降低只展示 cron 文本等场景的假阳性。
