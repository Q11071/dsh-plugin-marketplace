...
| 🧰 安装管理 | 筛选已安装插件，单独或批量更新、卸载、启用、停用，并可安全重启 DSH |
...
批量更新只会提交当前页面可见且符合自动更新条件的条目，隐藏在其他搜索或过滤结果中的插件不会被意外更新。所有任务会先进入队列，再按照选择顺序串行修改同一个 Profile，避免多个 pnpm 进程并发改写 manifest 或锁文件；单项失败会单独显示错误，但不会阻塞队列中的后续插件。
