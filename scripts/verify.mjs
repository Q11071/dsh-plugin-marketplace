...
// Exact-commit security results are generated by a no-secret workflow. New or
// changed commits remain guided until a result is available; pre-enforcement
// entries stay visible while the bounded initial backfill drains.
const securityReport = JSON.parse(readFileSync(path.join(root, 'registry', 'security-report.json'), 'utf8'))
if (!validSecurityReportRoot(securityReport) || securityReport.total !== securityReport.results.length) {
  throw new Error('security report root contract invalid')
}
const securityNames = new Set()
const currentSecurity = []
for (const row of securityReport.results) {
  if (!validSecurityResult(row)) throw new Error('security report result invalid: ' + String(row?.repository))
  const key = row.repository.toLocaleLowerCase()
  if (!registryNames.has(key)) throw new Error('security report references an unpublished repository: ' + row.repository)
  if (securityNames.has(key)) throw new Error('security report repeats repository ' + row.repository)
  securityNames.add(key)
  const plugin = registry.plugins.find(candidate => candidate.fullName.toLocaleLowerCase() === key)
  if (plugin.verifiedCommit === row.verifiedCommit) currentSecurity.push(row)
}
for (const plugin of registry.plugins) {
  const reason = securityGateReason(plugin, securityReport)
  if (reason !== null && plugin.install.mode !== 'guided') {
    throw new Error('security-gated Registry row is not guided: ' + plugin.fullName)
  }
}
const expectedSecuritySummary = {
  ...
  pending: registry.plugins.length - currentSecurity.length,
}
for (const [key, value] of Object.entries(expectedSecuritySummary)) {
